...
const ProductSidebar = () => {
  ...
  //   navigate(`?${newQueryParams.toString()}`);
  // };
  // const handleSubcategoryChange = (subcategoryId, parentCategoryName) => {
  //   setSelectedSubcategory(subcategoryId);
  //   setCategoryName(parentCategoryName); // Set the parent category name
  //   const newQueryParams = new URLSearchParams(location.search);
  //   newQueryParams.set("subcategory", subcategoryId);
  //   navigate(`?${newQueryParams.toString()}`);
  // };
  const handleSubcategoryChange = (subcategoryId, parentCategoryId, parentCategoryName) => {
    setSelectedSubcategory(subcategoryId);
    setSelectedCategory(parentCategoryId); // Set the parent category as active
    setCategoryName(parentCategoryName);
    
    const newQueryParams = new URLSearchParams(location.search);
    newQueryParams.set("subcategory", subcategoryId);
    newQueryParams.set("category", parentCategoryId); // Include parent category in URL
    navigate(`?${newQueryParams.toString()}`);
                        ...
                        expandedCategories[category._id] && (
                          <div className={styles.subcategories}>
                            {category.subcategories.map((subcategory) => (
                              <p
                                key={subcategory._id}
                                onClick={() => handleSubcategoryChange(subcategory._id, category.name)}
                                className={selectedSubcategory === subcategory._id ? styles.activeSubcategory : ""}
                              >
                                {subcategory.name}
                              </p>
  ...
  expandedCategories[category._id] && (
    <div className={styles.subcategories}>
      {category.subcategories.map((subcategory) => (
        <p
          key={subcategory._id}
          onClick={() => handleSubcategoryChange(subcategory._id, category._id, category.name)}
          className={selectedSubcategory === subcategory._id ? styles.activeSubcategory : ""}
        >
          {subcategory.name}
        </p>
