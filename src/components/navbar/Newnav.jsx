	...
	useEffect(() => {
		...
		const handleScroll = () => {
			if (window.scrollY > 200) {
				setIsBottomSectionVisible(false); // Hide the section after scrolling 500px
			} else {
				setIsBottomSectionVisible(true); // Show the section when scrolled back above 500px
			}
		};
							...
							{categories.map((item, index) => {
								return (
									<div className='nav_cat_list_item' key={index} >
										<div className='nav_nav_cat_options' onClick={() => handleCategoryClick(item.nav)}  >
											{item.name}
										</div>
									</div>
								)
